 ...
 * <p>
 * <p> User interfaces and operating systems use system-dependent 依赖的 <em>pathname
 * strings</em> to name files and directories.  This class presents an
 * abstract, system-independent 独立的 view of hierarchical pathnames.  An
 ...
 * in an abstract pathname denotes 为..的符号 a directory; the last name may denote
 * either a directory or a file.  The <em>empty</em> abstract pathname has no
 * prefix and an empty name sequence.
 * <p>
 * <p> The conversion 转换 of a pathname string to or from an abstract pathname is
 * inherently 固有的 天性的 system-dependent.  When an abstract pathname is converted into a
 * pathname string, each name is separated from the next by a single copy of
 * the default <em>separator 分割器 character</em>.  The default name-separator
 ...
 * denotes.  A relative pathname, in contrast, must be interpreted 解释 in terms of 根据
 ...
 * and drive specifiers 驱动说明符 , root directories and UNC pathnames on Microsoft Windows platforms,
 * as follows:
 * <p>
 * <ul>
 * <p>
 ...
 * prefix of a UNC Universal Naming Conversion 通用命名标准 pathname is <code>"\\\\"</code>; the hostname and the share
 ...
 * then that object resides in a <i>partition 分区 </i>.  A partition is an
 * operating system-specific portion 一部分 of storage for a file system.  A single
 ...
 * restrictions are collectively 共同的 known as <i>access permissions</i>.  The file
 * system may have multiple sets of access permissions on a single object.
 * For example, one set may apply to the object's <i>owner</i>, and another
 * may apply to all other users.  The access permissions on an object may
 * cause some methods in this class to fail.
 * <p>
 * <p> Instances of the <code>File</code> class are immutable 不可改变的 ; that is, once
...
public class File
    ...
    /**
     * This abstract pathname's normalized 标准化 pathname string. A normalized
     * pathname string uses the default name-separator character and does not
     * contain any duplicate 重复的 or redundant 多余的 separators.
     *
     * @serial
     */
    private String path;
    ...
    /**
     * Check if the file has an invalid path. Currently, the inspection 检查 of
     * a file path is very limited 有限的 , and it only covers Nul character check.
     * Returning true means the path is definitely 明确的 invalid/garbage 垃圾 . But
    ...
    /**
     * Internal constructor for already-normalized pathname strings.
     * The parameter order is used to disambiguate 消除歧义 this method from the
     ...
     * </tt></blockquote>
     * <p>
     * so long as 只要 the original abstract pathname, the URI, and the new abstract
     ...
     * @throws IllegalArgumentException If the preconditions 前提 on the parameter do not hold
     * @see #toURI()
     * @see java.net.URI
     * @since 1.4
     */
    ...
    /**
     * Returns the canonical 规范的 pathname string of this abstract pathname.
     * <p>
     * <p> A canonical pathname is both absolute and unique.  The precise 清晰地
     * definition of canonical form is system-dependent.  This method first
     * converts this pathname to absolute form if necessary, as if by invoking the
     * {@link #getAbsolutePath} method, and then maps it to its unique form in a
     * system-dependent way.  This typically involves removing redundant names
     * such as <tt>"."</tt> and <tt>".."</tt> from the pathname, resolving
     * symbolic 象征的 links (on UNIX platforms), and converting drive letters to a
     ...
     *
     * @return A URL object representing the equivalent 相等 file URL
     ...
     * <p>
     * <p> Note that when this abstract pathname represents a UNC (Universal Naming Convention) pathname then
     ...
     * pathname be deleted when the virtual machine terminates 结束 .
     ...
     * use the most appropriate 合适的 out of:
     * <ul>
     * <li>Use a {@code finally} clause 从句 分句 to manually invoke {@link #delete}.
     * <li>Maintain your own set of files to delete, and process it at an appropriate point
     * in your application's lifecycle.
     * <li>Use the Unix trick 技巧 of deleting the file as soon as all readers and writers have
     ...
     * but some provide more precision.  The argument will be truncated 删节的 to fit
     * the supported precision.  If the operation succeeds and no intervening 发生于其间的
    ...
    /**
     * Returns the number of unallocated 未分配的 bytes in the partition <a
     * href="#partName">named</a> by this abstract path name.
     * <p>
     * <p> The returned number of unallocated bytes is a hint, but not
     * a guarantee, that it is possible to use most or any of these
     * bytes.  The number of unallocated bytes is most likely to be
     * accurate 精确的 immediately after this call.  It is likely to be made
     ...
     * estimate 估计 of how much new data can actually be written than {@link
     * #getFreeSpace}.
     * <p>
     ...
     * and regardless of any quota 配额 or other restrictions that might apply to the user.
    ...
    private static File generateTempFile(String prefix, String suffix, File dir)
            throws IOException {
        // Android-changed: Use Math.randomIntInternal. This (pseudo 假的 ) random number
    ...
    /**
     * Compares two abstract pathnames lexicographically 字典顺序 .  The ordering
     * defined by this method depends upon the underlying system.  On UNIX
     * systems, alphabetic case is significant 重要的 in comparing pathnames; on Microsoft Windows
     * systems it is not.
     *
     ...
     * pathname is equal to the exclusive 专用的 单独的 <em>or</em> of the hash code
     ...
     * in case the path is reconstituted 再造的 on a different host type.
     * <p>
     *
