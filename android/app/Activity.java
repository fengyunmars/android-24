 ...
 * modularize 模块化 their code, build more sophisticated 精致的  user interfaces for larger
 ...
 *         or transparent 透明的 activity has focus on top of your activity), it
 ...
 *     <li>If an activity is completely obscured 遮盖  by another activity,
 ...
 * The square 正方形的  rectangles represent callback methods you can implement to
 ...
 * that impact 影响  your UI, and unregister it in onStop() when the user no
 ...
 * the device goes to sleep, when an activity result is delivered 交付 , when a new
 * intent is delivered -- so the code in these methods should be fairly
 * lightweight.
 * </ul>
 *
 * <p>The entire lifecycle of an activity is defined by the following
 * Activity methods.  All of these are hooks 钩子  that you can override
 ...
 * </pre>
 *
 * <p>In general the movement 运动  through an activity's lifecycle looks like
 * this:</p>
 *
 ...
 *
 * <p class="note">Be aware that these semantics 语义学  will change slightly between
 ...
 * impacts 影响  when {@link #onSaveInstanceState(Bundle)} may be called (it may be
 ...
 * {@link #onStop}, and {@link #onDestroy} as appropriate 视情况而定 .  If the activity
 ...
 * to return data back to its parent.  It must always supply 提供 a result code,
 ...
 * "edit in place" user model.  That is, any edits a user makes are effectively 有效地
 * made immediately 立即  without requiring an additional confirmation 确认  step.
 ...
 *             that point this e-mail will now appear in the list of drafts 草稿箱 .</p>
 ...
 *
 * <p>Here is an excerpt 摘录 from a calendar activity that stores the user's
 ...
 * <h3>Permissions</h3>
 *
 * <p>The ability to start a particular Activity can be enforced 执行  when it is
 * declared in its
 ...
 * Lifecycle</a>, the decision 决定  about which process to remove is intimately 熟悉的
 ...
 * Its process will only be killed as a last resort 万不得已 , if it uses more memory
 ...
 * the user and has been paused) is no longer critical 关键的 , so the system may
 * safely kill its process to reclaim 开拓  memory for other foreground or
