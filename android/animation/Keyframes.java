...
public interface Keyframes extends Cloneable {
     ...
     * both of which assume 认为 their own evaluator 求值程序 to speed up 加速 calculations with those primitive
     * types.
     *
     ...
     * function maps the input fraction to the appropriate keyframe interval 间隔 and a fraction
     * between them and returns the interpolated value. Note that the input fraction may fall
     * outside the [0-1] bounds, if the animation's interpolator made that happen (e.g., a
     * spring interpolation that might send the fraction past 1.0). We handle this situation by
     * just using the two keyframes at the appropriate 适当的 end when the value is outside those bounds.
     *
