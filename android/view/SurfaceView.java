...
/**
 * Provides a dedicated 专用的  drawing surface embedded inside of a view hierarchy.
 * You can control the format of this surface and, if you like, its size; the
 * SurfaceView takes care of placing the surface at the correct location on the
 * screen 
 *
 * <p>The surface is Z ordered 纵深排序 的  so that it is behind the window holding its
 * SurfaceView; the SurfaceView punches a hole 洞  in its window to allow its
 * surface to be displayed. The view hierarchy will take care of correctly
 * compositing 合成 with the Surface any siblings of the SurfaceView that would
 * normally appear on top of it. This can be used to place overlays 覆盖图  such as
 * buttons on top of the Surface, though note however that it can have an
 * impact on performance since a full alpha-blended 混合的  composite will be performed
 * each time the Surface changes.
 *
 * <p> The transparent 透明的  region that makes the surface visible is based on the
