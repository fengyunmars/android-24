 ...
 * SoundPool library uses the MediaPlayer service to decode 解码  the audio 音频
 * into a raw 16-bit PCM 脉冲编码调制 mono 单声道  or stereo 立体声  stream. This allows applications
 * to ship 运送  with compressed 压缩的  streams without having to suffer 忍受  the CPU load
 * and latency 潜在  of decompressing 解压缩  during playback.</p>
 *
 * <p>In addition to 除了 .. 之外 low-latency 低延时 playback, SoundPool can also manage the number
 ...
 * number of streams helps to cap 覆盖  CPU loading and reducing the likelihood 可能性  that
 ...
 * the sound to play at its original frequency 频率  (resampled, if necessary,
 ...
 * the SoundPool was created. In this case, the stream allocator 分配器 will stop
 ...
 * for decompressing 减压  the audio to raw PCM format before they are needed for
 * playback.</p>
 *
 ...
 * paused or resumed, and the application can also alter the pitch 音高  by
 * adjusting the playback rate in real-time for doppler or synthesis 综合的
 * effects.</p>
 *
...
public class SoundPool {
     ...
     * characteristics 特征 :
     *
     * @param maxStreams the maximum number of simultaneous 同时的  streams for this
     *                   SoundPool object
     * @param streamType the audio stream type as described in AudioManager 
     *                   For example, game applications will normally use
     *                   {@link AudioManager#STREAM_MUSIC}.
     * @param srcQuality the sample-rate converter quality 质量 . Currently has no
