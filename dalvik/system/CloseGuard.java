...
/**
 * CloseGuard is a mechanism for flagging 逐渐衰退 implicit 不言明[含蓄]的 finalizer cleanup of
 * resources that should have been cleaned up by explicit 明确的 close
 * methods (aka 又叫做 "explicit termination methods" in Effective 有效的 Java).
 * <p>
 ...
 * allocated 分派 after object construction, CloseGuard protection can
 * be deferred 延期的 . For example: <pre>   {@code
 *   class Bar {
 *
 *       private final CloseGuard guard = CloseGuard.get();
 *
 ...
 * abrupt 突然的 termination 结束 of the constructor will mean that the user will
 * not have a reference to the object to cleanup explicitly. When used
 * in a method, the call to {@code open} should occur just after
 * resource acquisition 获得 .
 *
 * @hide
 */
public final class CloseGuard {
    ...
    /**
     * Default Reporter which reports CloseGuard violations 违反 to the log.
     */
